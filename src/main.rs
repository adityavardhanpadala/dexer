...
impl Dex<'_> {
    ...
    fn new(path: &String) {
        ...
        let type_id_items = get_u32_items(&dexfile, header.type_ids_off as usize, header.type_ids_size as usize);
        let type_map: std::collections::HashMap<u32, String> = type_id_items
            .iter()
            .filter_map(|&idx| {
                string_map.get(&string_id_items[idx as usize]).map(|s| (idx, s.clone()))
            })
            .collect();
       
        let proto_id_items = get_items::<proto_id_item>(&dexfile, header.proto_ids_off as usize, header.proto_ids_size as usize);
        
        println!("{:#x?}", proto_id_items[0]);

        let field_id_items = get_items::<field_id_item>(&dexfile, header.field_ids_off as usize, header.field_ids_size as usize);

        println!("{:#x?}", field_id_items[0]);

        let method_id_items = get_items::<method_id_item>(&dexfile, header.method_ids_off as usize, header.method_ids_size as usize);

        println!("{:#x?}", method_id_items[0]);

        let class_def_items = get_items::<class_def_item>(&dexfile, header.class_defs_off as usize, header.class_defs_size as usize);
